    ...
    <Dialog v-model:open="editDialog">
      <DialogContent class="sm:max-w-[600px]">
        <DialogHeader>
          ...
          <DialogDescription> Guruh ma'lumotlarini yangilash </DialogDescription>
        </DialogHeader>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Bu amalni ortga qaytarib bo'lmaydi. Bu guruh butunlay o'chiriladi
            va barcha bog'langan talabalar ro'yxati va davomat
            yozuvlari ham o'chiriladi.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
  ...
  if (search.value) {
    ...
    result = result.filter(
      (group) =>
        ...
        getCourseTitle(group.level_id).toLowerCase().includes(searchLower) ||
        getTeacherName(group.teacher_id).toLowerCase().includes(searchLower)
    );
  }
...
const loadTeachers = async () => {
  ...
  try {
    ...
    toast({
      title: "Xatolik",
      description: "O'qituvchilarni yuklashda xatolik. Qaytadan urinib ko'ring.",
      variant: "destructive",
    });
  } finally {
...
const updateGroup = async () => {
  ...
  try {
    ...
    const index = groups.value.findIndex((g) => g.id === response.id);
    if (index !== -1) {
      groups.value[index] = {
        ...response,
      };
...
const updateGroupStatus = async () => {
  ...
  try {
    ...
    toast({
      title: "Xatolik",
      description: "Guruh holatini yangilashda xatolik. Qaytadan urinib ko'ring.",
      variant: "destructive",
    });
  } finally {
    isUpdating.value = false;
...
const getTeacherName = (teacherId: string): string => {
  ...
  return teacher ? `${teacher.first_name} ${teacher.last_name}` : "Tayinlanmagan";
};
