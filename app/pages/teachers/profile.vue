    ...
    <Card>
        ...
        <Table>
          ...
          <TableBody v-if="!loading && paginatedProfiles.length > 0">
            <TableRow v-for="profile in paginatedProfiles" :key="profile.id">
              ...
              <TableCell>
                <Badge :variant="profile.payment_type === 'percentage' ? 'default' : 'secondary'">
                  {{ profile.payment_type === 'percentage' ? 'Foiz' : 'Qat\'iy summa' }}
                </Badge>
              </TableCell>
              <TableCell>
      ...
      <CardFooter class="flex items-center justify-between">
        <div class="text-sm text-muted-foreground">
          Ko'rsatilmoqda {{ paginationStart }} - {{ paginationEnd }} / {{ filteredProfiles.length }} ta profil
        </div>
        <Pagination
      ...
      <DialogContent class="sm:max-w-[500px]">
        ...
        <div class="space-y-4 py-4" v-if="selectedProfile">
          <div class="grid grid-cols-3 gap-4">
            ...
            <span class="col-span-2">{{ selectedProfile.teacher_name || 'N/A' }}</span>
          </div>
          <div class="grid grid-cols-3 gap-4">
            <span class="font-semibold">To'lov turi:</span>
            <span class="col-span-2">
              <Badge :variant="selectedProfile.payment_type === 'percentage' ? 'default' : 'secondary'">
                {{ selectedProfile.payment_type === 'percentage' ? 'Foiz' : "Qat'iy summa" }}
              </Badge>
            </span>
          </div>
          <div class="grid grid-cols-3 gap-4">
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Haqiqatan ham bu profilni o'chirmoqchimisiz? Bu amalni ortga qaytarib bo'lmaydi.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel>Bekor qilish</AlertDialogCancel>
          <AlertDialogAction @click="deleteProfile">O'chirish</AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
  </div>
...
const filteredProfiles = computed(() => {
  return profiles.value.filter((profile) => {
    ...
    const searchMatch =
      profile.teacher_name?.toLowerCase().includes(searchQuery.value.toLowerCase()) ?? false;

    // Filter by payment type
    const typeMatch =
      paymentTypeFilter.value === "all" ||
      profile.payment_type === paymentTypeFilter.value;
...
const totalPages = computed(() => {
  return Math.max(1, Math.ceil(filteredProfiles.value.length / itemsPerPage));
});

const paginatedProfiles = computed(() => {
  const startIndex = (currentPage.value - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  return filteredProfiles.value.slice(startIndex, endIndex);
});
...
const fetchProfiles = async () => {
  loading.value = true;
  try {
    const response = await api.get<Teacher[]>(apiService.value, "/users/teachers");
    
    // Extract profiles from teachers and add teacher names
    const profilesWithNames = (response || [])
...
const saveProfile = async () => {
  ...
  if (profileForm.value.payment_value === null || profileForm.value.payment_value === undefined) {
  ...
  try {
    ...
    toast.error(error?.response?.data?.message || "Profilni saqlashda xatolik yuz berdi");
  }
};
...
const updateUrlParams = () => {
  router.push({
    query: {
      ...
      type: paymentTypeFilter.value !== "all" ? paymentTypeFilter.value : undefined,
    },
  });
};
